...
class AnalyticsService:
    ...
    async def get_dashboard_metrics(self) -> Dict[str, Any]:
        """Calcula todas las métricas cuantitativas, estadísticas y desglose de tecnologías."""
        ...
        # Avg AI Score
        avg_score_res = await self.db.execute(select(func.avg(Job.ai_score)).where(Job.ai_score.isnot(None)))
        ...
        # High match jobs
        high_res = await self.db.execute(select(func.count(Job.id)).where(Job.ai_score >= 80.0))
        ...
        techs_res = await self.db.execute(select(Job.technologies).where(Job.technologies.isnot(None)))
        all_techs = []
        ...
        # CRM Stages
        stages_res = await self.db.execute(select(ApplicationStage.stage, func.count(ApplicationStage.id)).group_by(ApplicationStage.stage))
