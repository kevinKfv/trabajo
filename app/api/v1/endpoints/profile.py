...
from fastapi import APIRouter, Depends, HTTPException, UploadFile, File, Form, status, BackgroundTasks, Header
...
async def _reset_and_trigger_scrape(db: AsyncSession, profile: UserProfile, background_tasks: BackgroundTasks, device_id: str):
    await db.execute(delete(SearchConfig).where(SearchConfig.device_id == device_id))
    ...
    async def _bg_scrape():
        async with AsyncSessionLocal() as session:
            ...
            await manager.run_all_active_search_configs(device_id=device_id)
...
async def get_profile(
    ...
    result = await db.execute(select(UserProfile).where(UserProfile.device_id == x_device_id).limit(1))
...
async def update_profile(
    ...
    if not profile:
        profile = UserProfile(device_id=x_device_id)
        db.add(profile)
    ...
    await _reset_and_trigger_scrape(db, profile, background_tasks, x_device_id)
    return profile
