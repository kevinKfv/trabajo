...
async def list_search_configs(db: AsyncSession = Depends(get_db)):
    """Lista todas las configuraciones de búsqueda personalizadas."""
    result = await db.execute(select(SearchConfig).order_by(SearchConfig.id.desc()))
...
async def create_search_config(
    ...
    config = SearchConfig(**config_in.model_dump())
    db.add(config)
    await db.commit()
